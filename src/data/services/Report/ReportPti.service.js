...
export class ReportPtiService {
  async execute({ page, limit, id_allocation_period, download }) {
    const allocations = await Allocation.findAndCountAll({
      ...
      order: [
        [{ model: Professional, as: 'professional' }, 'nm_professional', 'ASC'],
      ],
    ...
    const getAllocations = Promise.all(
      allocations.rows.map(async alloc => {
        const {
        ...
        return {
          nm_project:
            alloc.dataValues.product.dataValues.project_phase.dataValues.project
              .dataValues.nm_project,
          cd_sei:
            alloc.dataValues.product.dataValues.project_phase.dataValues.project
              .dataValues.cd_sei || '',
          nm_professional: professional.dataValues.nm_professional,
          nm_city:
            alloc.dataValues.product.dataValues.project_phase.dataValues.project
              .dataValues.city.dataValues.nm_city,
          nm_product: alloc.dataValues.product.dataValues.nm_product,
          tp_action_picture:
            (alloc.dataValues.tp_action_picture === 0 && 'Não Definida') ||
            (alloc.dataValues.tp_action_picture === 1 && 'Produção Integral') ||
            (alloc.dataValues.tp_action_picture === 2 && 'Produção Parcial') ||
          ...
          cd_status:
            (productHistories[productHistories.length - 1].cd_status === 0 &&
              'Ag. Alocação') ||
            (productHistories[productHistories.length - 1].cd_status === 1 &&
              'Em Produção') ||
            (productHistories[productHistories.length - 1].cd_status === 2 &&
              'Em Análise') ||
            (productHistories[productHistories.length - 1].cd_status === 3 &&
              'Em Correção') ||
            (productHistories[productHistories.length - 1].cd_status === 4 &&
              'Em Análise de Correção') ||
            (productHistories[productHistories.length - 1].cd_status === 5 &&
              'Concluído'),
        };
      })
    );
    ...
    if (download) {
      ...
      const professionalList = await getAllocations;
