...
var _database = require('../../database');
 ...
 class ReportContactService {
  async execute({
    ...
    const [count] = await _database.sequelize.query(
      `SELECT 
    ...
    const formattedValues = results.map(
      ({
        ...
        return {
          ...
          nm_city,
          dt_contatct: dtContatct
            ? _datefns.format.call(void 0, new Date(dtContatct), 'dd/MM/yyyy')
            : '',
          ...
          nu_phone,
          dt_agreed_feedback: dtAgreedFeedback
            ? _datefns.format.call(void 0, new Date(dtAgreedFeedback), 'dd/MM/yyyy')
            : '',
          dt_feedback: dt_feedback
            ? _datefns.format.call(void 0, new Date(dt_feedback), 'dd/MM/yyyy')
            : '',
        };
      }
    );
...
} exports.ReportContactService = ReportContactService;
