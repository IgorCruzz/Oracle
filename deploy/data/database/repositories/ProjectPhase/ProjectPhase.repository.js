...
var _models = require('../../models');
 ...
 class ProjectPhaseRepository {
  async createManyProjectPhases(data) {
    const projectPhase = await _models.Project_phase.bulkCreate(data);
  ...
  async createProjectPhase(data) {
    ...
    const findProject = await _models.Project_phase.findAll({
      where: {
        id_project,
      },
    });

    const count = await _models.Project_phase.findAndCountAll({
      where: {
        id_project,
      },
    });
    ...
    const createdProjectPhase = await _models.Project_phase.create({
    ...
    return await _models.Project_phase.findOne({
      where: {
  ...
  async verifyProjectLogicDeleted({ id_project }) {
    return await _models.Project_phase.findAll({
      include: [
        {
  ...
  async findProjectPhases({ page, limit, id_project }) {
    if (limit && page) {
      return await _models.Project_phase.findAndCountAll({
  ...
  async deleteProjectPhase({ id_project_phase }) {
    await _models.Project_phase.destroy({
  ...
  async updateTechnicalManagerArea(id_project_phase, data) {
    ...
    const ProjectPhase = await _models.Project_phase.findOne({
      where: {
        id_project_phase,
      },
    });
...
} exports.ProjectPhaseRepository = ProjectPhaseRepository;
